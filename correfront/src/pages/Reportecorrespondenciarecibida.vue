  ...
  <div class="col-12">
    <q-table :filter="filter" dense square bordered flat title="Historial de correspondencia" :rows-per-page-options="[0]" :columns="columns" :rows="datos">
      ...
      <template v-slot:body-cell-datos="props" >
        <q-td auto-width :props="props">
          <div class="">DE: {{props.row.user.name}} | <q-badge color="blue">
             <div class=""> {{props.row.user.unit.nombre}}</div>
          </q-badge> </div>
...
export default {
  ...
  methods:{
    consulta(){
      this.$q.loading.show()
      this.$axios.post(process.env.API+'/reportecorrespondenciarecibida',{
        fecha1:this.fecha1,
        fecha2:this.fecha2,
      }).then(res=>{
        this.datos=[]
    ...
    impresion(){
      ...
      this.datos.forEach(d=>{
        ...
        doc.setFontSize(12);
         doc.setFont('times', 'bold');
        doc.text(d.mail.citecontrol.toString().substr(0,15),23,38+i*34,'center')
        ...
        doc.text('ESTADO: '+d.estado,82,35+i*34,'left')
        doc.setFont('times', 'normal');
        doc.text('REF: '+d.mail.ref.substr(0,50),82,40+i*34,'left')
        doc.text(d.mail.ref.substr(50,50),82,45+i*34,'left')
        doc.text(d.mail.ref.substr(100,50),82,50+i*34,'left')
